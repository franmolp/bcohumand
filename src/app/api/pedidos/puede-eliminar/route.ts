...
export async function GET() {
  ...
  const [puedeRes, usuariosRes] = await Promise.all([
    supabaseAdmin.from('pedidos_puede_eliminar').select('usuario_id'),
    supabase.from('usuarios').select('id, nombre, foto_perfil').eq('estado_cuenta', 'activo').order('nombre'),
  ])

  return NextResponse.json({
    puedeEliminar: (puedeRes.data ?? []).map(e => e.usuario_id),
