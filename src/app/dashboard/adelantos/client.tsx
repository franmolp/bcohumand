...
import {
  IconDollar, IconPlus, IconX, IconCheck, IconSettings, IconAlertCircle, IconChevronLeft, IconChevronRight,
...
export default function AdelantosClient({ user }: { user: SessionUser }) {
  ...
  useEffect(() => {
    fetch('/api/adelantos/config').then(r => r.json()).then(d => {
      setConfig(d)
      setConfigEdit(d)
    }).catch(() => {})
  }, [])
  ...
  useEffect(() => {
    if (!isAdmin) return
    fetch('/api/adelantos/usuarios').then(r => r.json()).then(d => {
  ...
  async function handleAction() {
    ...
    const adelanto = adelantos.find(a => a.id === actionId)
    try {
      const res = await fetch('/api/adelantos', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...
          monto_aprobado: actionType === 'approve' ? Number(actionMonto || adelanto?.monto || 0) : null,
    ...
    try {
      const res = await fetch('/api/adelantos', {
        ...
        body: JSON.stringify({
          ...
          empleado_nombre: createNombre,
          monto: Number(createMonto),
          comentario_admin: createComment || null,
        }),
      })
      const data = await res.json()
      ...
      <div className="p-4 lg:p-6 max-w-2xl mx-auto">
        ...
        <div className="flex gap-1 p-1 bg-gray-100 rounded-xl mb-5">
          {(['pendientes', 'mes', 'ajustes'] as const).map(t => (
            ...
            >
              {t === 'pendientes'
                ? `Pendientes${adelantos.length && tab === 'pendientes' ? ` (${adelantos.length})` : ''}`
                : t === 'mes' ? 'Por mes'
              ...
              <div key={a.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-3">
                ...
                {actionId === a.id ? (
                  <div className="border-t border-gray-100 pt-3 space-y-3">
                    {actionType === 'approve' && (
                      <div>
                        <label className="text-[11px] text-gray-400 font-semibold mb-1.5 block uppercase tracking-wide">Monto a aprobar</label>
                    ...
                    <div>
                      <label className="text-[11px] text-gray-400 font-semibold mb-1.5 block uppercase tracking-wide">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => { setActionId(null); setActionType(null); setActionMonto(''); setActionComment('') }}
                        className="flex-1 py-2.5 border border-gray-200 rounded-xl text-[13px] text-gray-500 cursor-pointer hover:bg-gray-50"
                      >
                        Cancelar
                      </button>
                      <button
                        ...
                        className={`flex-1 py-2.5 rounded-xl text-[13px] font-semibold text-white cursor-pointer disabled:opacity-60 ${
                          actionType === 'approve' ? 'bg-green-500 hover:bg-green-600' : 'bg-red-500 hover:bg-red-600'
                        }`}
                      >
                        {actionSubmitting ? '...' : actionType === 'approve' ? 'Confirmar aprobación' : 'Confirmar rechazo'}
                      </button>
                    </div>
                  </div>
                ) : (
                  <div className="flex gap-2 border-t border-gray-100 pt-3">
                    <button
                      onClick={() => { setActionId(a.id); setActionType('reject'); setActionMonto(''); setActionComment('') }}
                    ...
                    <button
                      onClick={() => { setActionId(a.id); setActionType('approve'); setActionMonto(String(a.monto)); setActionComment('') }}
        ...
        {tab === 'mes' && (
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => setMesFiltro(prevMes(mesFiltro))}
                className="p-2 rounded-xl border border-gray-200 cursor-pointer hover:bg-gray-50 transition-colors"
              >
              ...
              <button
                onClick={() => setMesFiltro(nextMes(mesFiltro))}
                className="p-2 rounded-xl border border-gray-200 cursor-pointer hover:bg-gray-50 transition-colors"
              >
        ...
        {tab === 'ajustes' && (
          <form onSubmit={handleSaveConfig} className="space-y-4">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-5">
              ...
              <div>
                <p className="text-[13px] font-semibold text-gray-700 mb-3">Montos permitidos</p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[11px] text-gray-400 font-semibold uppercase tracking-wide mb-1.5 block">Mínimo ($)</label>
                    <input
                      type="number"
                      value={configEdit.monto_minimo}
                  ...
                  <div>
                    <label className="text-[11px] text-gray-400 font-semibold uppercase tracking-wide mb-1.5 block">Máximo ($)</label>
                    <input
                      type="number"
                      value={configEdit.monto_maximo}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      type="number"
                      value={configEdit.dia_habilitacion}
                  ...
                  <div>
                    <label className="text-[11px] text-gray-400 font-semibold uppercase tracking-wide mb-1.5 block">Máx. adelantos/mes</label>
                    <input
                      type="number"
                      value={configEdit.max_por_mes}
            ...
            {configMsg && (
              <p className="text-[13px] text-green-600 text-center font-medium">{configMsg}</p>
            )}

            <button
              type="submit"
              disabled={configSaving}
              className="w-full py-3 bg-[image:var(--gradient)] text-white text-[14px] font-semibold rounded-xl cursor-pointer disabled:opacity-60"
            >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setShowCreate(false)}>
            ...
            <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-5 space-y-4" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setShowCreate(false)} className="p-1 text-gray-400 cursor-pointer hover:text-gray-600">
                  <IconX size={18} />
                </button>
              </div>
              ...
              <form onSubmit={handleCreate} className="space-y-3">
                  ...
                  {usuarios.length > 0 ? (
                    <select
                      ...
                      onChange={e => {
                        ...
                        setCreateNombre(u?.nombre ?? '')
                      }}
                      required
                      className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-[14px] outline-none focus:border-[var(--primary)] bg-white cursor-pointer"
                    >
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  <button
                    ...
                    className="flex-1 py-2.5 bg-[image:var(--gradient)] text-white text-[14px] font-semibold rounded-xl cursor-pointer disabled:opacity-60"
                  >
  ...
  const thisMesUsados = adelantos.filter(a =>
    a.created_at.slice(0, 7) === currentMes && a.estado !== 'rejected'
  ).length
      ...
      {!showForm ? (
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={() => { setShowForm(false); setFormError('') }} className="p-1 text-gray-400 cursor-pointer">
              <IconX size={16} />
            </button>
          </div>
          ...
          <div>
            <label className="text-[11px] text-gray-400 font-semibold uppercase tracking-wide mb-1.5 block">
              Monto · {fmtMonto(config.monto_minimo)} mín — {fmtMonto(config.monto_maximo)} máx
            </label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full border border-gray-200 rounded-xl pl-7 pr-3 py-3 text-[16px] outline-none focus:border-[var(--primary)]"
                autoFocus
              />
            </div>
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ej: Para pagar el alquiler"
              className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-[14px] outline-none focus:border-[var(--primary)]"
            />
          </div>
                    ...
                    <div key={a.id} className="px-4 py-3.5 space-y-1.5">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <p className="text-[17px] font-bold text-[var(--text)]">
                          {fmtMonto(a.monto_aprobado ?? a.monto)}
                        </p>
                      </div>
                      ...
                      <p className="text-[11px] text-gray-400">{fmtFecha(a.created_at)}</p>
                      {a.comentario_empleado && (
                        <p className="text-[12px] text-gray-500 leading-relaxed">{a.comentario_empleado}</p>
                      )}
                      {a.comentario_admin && (
                        <p className="text-[12px] text-[var(--primary)] italic">"{a.comentario_admin}"</p>
                      )}
                    </div>
                  ))}
                </div>
