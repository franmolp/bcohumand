...
export default function AdelantosClient({ user }: { user: SessionUser }) {
  ...
  const itemsActuales = gruposPeriodo[0].items
  ...
  return (
    <div className="py-4 fade-in space-y-4">
      ...
      {loading ? (
      ...
      ) : gruposPeriodo.every(g => g.items.length === 0) ? (
        <div className="text-center py-12">
        ...
        <div className="space-y-5">
          {gruposPeriodo.filter(g => g.items.length > 0).map(g => (
            <div key={g.key} className="space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide">{g.label}</p>
                {g.total > 0 && <p className="text-[11px] font-semibold text-gray-400">Total: {fmtMonto(g.total)}</p>}
              </div>
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="divide-y divide-gray-50">
                  {g.items.map(a => (
